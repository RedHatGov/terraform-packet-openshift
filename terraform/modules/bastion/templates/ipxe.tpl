...
kernel $${coreos-url}/rhcos-$${release}.$${zstream}-$${arch}-installer-kernel-$${arch} $${console} ip=dhcp nameserver=1.1.1.1 nameserver=8.8.8.8 nomodeset rd.peerdns=0 rd.neednet=1 initrd=rhcos-$${release}.$${zstream}-$${arch}-installer-initramfs.$${arch}.img coreos.inst=yes coreos.inst.install_dev=sda coreos.inst.image_url=$${coreos-img} coreos.inst.ignition_url=http://${ bastion_ip }:8080/${ node_type }-append.ign
